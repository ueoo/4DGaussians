...
class Camera:
    ...
    def get_pixel_centers(self):
        ...
        xx, yy = np.meshgrid(
            np.arange(self.image_size_x, dtype=self.dtype), np.arange(self.image_size_y, dtype=self.dtype)
        )
    ...
    def scale(self, scale: float):
        ...
        new_camera = Camera(
            ...
            image_size=np.array((int(round(self.image_size[0] * scale)), int(round(self.image_size[1] * scale)))),
        )
        return new_camera
